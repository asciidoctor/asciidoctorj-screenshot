...
width:: if set the value is passed unchanged to the backend (i.e. HTML or PDF). The behavior is the same as setting this attribute on an `image` block.
height:: if set the value is passed unchanged to the backend (i.e. HTML or PDF). The behavior is the same as setting this attribute on an `image` block.
